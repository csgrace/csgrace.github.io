...
class ProblemSolvingAgent:
        ...
        while stack:
            ...
            if current == goal_pos:
                ...
                while node is not None:
                    ...
                    node = parents[node]
                return path, visited
            for neighbour, _ in self.neighbours_of(obstacles, current):
                if neighbour not in visited_set and neighbour not in parents:
        ...
        while heap:
            ...
            if current == goal_pos:
                ...
                while node is not None:
                    ...
                    node = parents[node]
                return path, visited
            for neighbour, move_cost in self.neighbours_of(obstacles, current):
    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        import heapq
        heap = [] # g+h
        heapq.heappush(heap, (self.euclidean_distance(start_pos, goal_pos), 0, start_pos))
        parents = {start_pos: None}
        g_cost = {start_pos: 0}
        visited_set = set()

        while heap:
            f, current_g, current = heapq.heappop(heap)
            if current in visited_set:
                continue
            visited.append(current)
            visited_set.add(current)
